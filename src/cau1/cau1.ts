const array = [1, 6, 10, 9, 7, 3, 2, 4, 13, 11, 30, 18];
...
function findPair1(A: number[], X: number) {
  const B = A.map(a => {
    return X - a;
  });
  ...
  for (let i = 0; i < A.length; i++) {
    for (let k = 0; k < B.length; k++) {
      if (A[i] === B[k]) console.log("Cặp vị trí thỏa mãn: (%d, %d) - (%d, %d)", i, k, A[i], A[k]);
    }
  }
}

// ! Cách 2: Em cộng lần lượt
function findPair2(A: number[], X: number) {
  for (let i = 0; i < A.length - 1; i++) {
    for (let k = i + 1; k < A.length; k++) {
      if (A[i] + A[k] === X) console.log("Cặp vị trí thỏa mãn: (%d, %d) - (%d, %d)", i, k, A[i], A[k]);
    }
  }
}
